...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubUsers = currentUser.role === 'admin'
      ? subscribeToUsers((usersList) => setAllUsers(usersList))
      : () => undefined;
  ...
  const handleSaveMealRecord = (record: Omit<DailyMealRecord, 'id' | 'totalMeals' | 'createdAt'> & { id?: string; createdAt?: string }) => {
    ...
    saveMealRecordToFirestore(savedRecord).catch((err) => showToast(err.message || 'Erro ao salvar refeição.', 'warning'));
  };

  const handleDeleteMealRecord = (id: string) => {
    const updated = deleteStoredMealRecord(meals, id);
    setMeals(updated);
    deleteMealRecordFromFirestore(id).catch((err) => showToast(err.message || 'Erro ao excluir refeição.', 'warning'));
  };

  const handleSelectLocalRole = (_role: UserRole) => {
    showToast('A alteração de perfil é exclusiva do administrador.', 'warning');
  };

  const handleResetData = () => {
    showToast('A redefinição automática está desativada para proteger o histórico e o estoque real.', 'warning');
  };
  ...
  const handleAddEntry = async (
    product: Product, quantity: number, entryType: EntryType, supplierOrDonor: string,
    receivedBy: string, date: string, time: string, notes: string
  ) => {
    try {
  ...
  const handleAddExit = async (
    product: Product, quantity: number, sector: Sector, retrievedBy: string,
    deliveredBy: string, date: string, time: string, notes: string
  ) => {
    try {
  ...
  const handleAddBatchExit = async (
    items: Array<{ product: Product; quantity: number }>, sector: Sector,
    retrievedBy: string, deliveredBy: string, date: string, time: string, notes: string
  ) => {
    try {
      const result = await executeBatchExitTransaction(
        items.map((item) => ({ productId: item.product.id, quantity: item.quantity })),
        sector, retrievedBy, deliveredBy, date, time, notes
      );
  ...
  const handleDeliverKit = async (
    kitToDeliver: DailyKit, retrievedBy: string, deliveredBy: string,
    date: string, time: string, saveAsDefault?: boolean
  ) => {
    try {
      if (saveAsDefault) {
  ...
  const handleForceSyncPhysicalInventory = async () => {
    showToast('A sincronização física automática foi desativada para proteger o estoque real.', 'warning');
  };
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-white">
        <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-3xl p-7 text-center shadow-2xl">
          <div className="mx-auto mb-4 w-14 h-14 rounded-2xl bg-amber-500/10 text-amber-400 flex items-center justify-center text-2xl">⏳</div>
          <h1 className="text-xl font-black">Acesso aguardando aprovação</h1>
          <p className="text-sm text-slate-400 mt-2">Olá, {currentUser.displayName}. Sua conta foi criada com segurança, mas ainda não recebeu uma permissão operacional.</p>
          <p className="text-xs text-slate-500 mt-3">{currentUser.email}</p>
          <button onClick={handleLogout} className="mt-6 px-5 py-3 rounded-xl bg-white text-slate-900 font-bold text-xs">Sair</button>
        </div>
      </div>
    );
  }
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'dashboard' && (
            <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
              ...
              <KpiCards products={products} movements={movements} />
              <CurrentStockOverview products={products} onOpenTimeline={handleOpenTimeline} onOpenEntry={handleOpenEntryModal} onOpenExit={handleOpenExitModal} userRole={currentUser.role} />
