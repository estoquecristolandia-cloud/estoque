...
import { subscribeToProducts, subscribeToMovements, subscribeToDailyKit, subscribeToUsers, subscribeToMeals, saveProductToFirestore, deleteProductFromFirestore, saveDailyKitToFirestore, saveMealRecordToFirestore, deleteMealRecordFromFirestore, syncInitialFirestoreData, executeEntryTransaction, executeExitTransaction, executeBatchExitTransaction, executeDailyKitTransaction, updateStockMovementTransaction, deleteStockMovementTransaction } from './services/firestoreService';
...
export default function App() {
  ...
  const [isDarkMode, setIsDarkMode] = useState<boolean>(() => localStorage.getItem('cristolandia_theme') !== 'light');
  ...
  useEffect(() => { document.documentElement.classList.toggle('dark', isDarkMode); localStorage.setItem('cristolandia_theme', isDarkMode ? 'dark' : 'light'); }, [isDarkMode]);
  ...
  useEffect(() => {
    ...
    const unsubUsers = currentUser.role === 'admin' ? subscribeToUsers((data) => setAllUsers(data)) : () => undefined;
    return () => { unsubProds(); unsubMovs(); unsubKit(); unsubMeals(); unsubUsers(); };
  }, [currentUser?.uid, currentUser?.role]);

  const showToast = (message: string, type: 'success' | 'warning' | 'info' = 'success') => type === 'success' ? toast.success(message) : type === 'warning' ? toast.warning(message) : toast.info(message);
  const handleSaveMealRecord = (record: Omit<DailyMealRecord, 'id' | 'totalMeals' | 'createdAt'> & { id?: string; createdAt?: string }) => { const { updatedMeals, savedRecord } = addOrUpdateMealRecord(meals, record); setMeals(updatedMeals); saveMealRecordToFirestore(savedRecord).catch((e) => showToast(e.message, 'warning')); };
  const handleDeleteMealRecord = (id: string) => { const updated = deleteStoredMealRecord(meals, id); setMeals(updated); deleteMealRecordFromFirestore(id).catch((e) => showToast(e.message, 'warning')); };
  ...
  const handleAddBatchExit = async (items: Array<{ product: Product; quantity: number }>, sector: Sector, retrievedBy: string, deliveredBy: string, date: string, time: string, notes: string) => {
    try { const result = await executeBatchExitTransaction(items.map((i) => ({ productId: i.product.id, quantity: i.quantity })), sector, retrievedBy, deliveredBy, date, time, notes); setProducts((prev) => prev.map((p) => result.updatedProducts.find((u) => u.id === p.id) || p)); setMovements((prev) => [...result.movements, ...prev.filter((m) => !result.movements.some((r) => r.id === m.id))]); showToast(`Saída de ${items.length} item(ns) realizada com sucesso para ${sector}!`); }
  ...
  const handleUpdateMovement = async (movementId: string, updatedData: Partial<StockMovement> & { productId: string; quantity: number; type: 'entrada' | 'saida' }) => {
    try { const result = await updateStockMovementTransaction(movementId, updatedData); setProducts((prev) => prev.map((p) => result.updatedProducts.find((u) => u.id === p.id) || p)); setMovements((prev) => prev.map((m) => m.id === result.movement.id ? result.movement : m)); showToast('Movimentação atualizada com sucesso!'); }
    catch (err: any) { showToast(err.message || 'Erro ao atualizar movimentação', 'warning'); }
  };
  const handleDeleteMovement = async (movementId: string) => {
    try { const result = await deleteStockMovementTransaction(movementId); setProducts((prev) => prev.map((p) => p.id === result.updatedProduct.id ? result.updatedProduct : p)); setMovements((prev) => prev.filter((m) => m.id !== result.deletedMovementId)); showToast('Movimentação excluída e saldo estornado!', 'info'); }
  ...
  const handleDeliverKit = async (kitToDeliver: DailyKit, retrievedBy: string, deliveredBy: string, date: string, time: string, saveAsDefault?: boolean) => {
    try { if (saveAsDefault) { await saveDailyKitToFirestore(kitToDeliver); setDailyKit(kitToDeliver); saveDailyKit(kitToDeliver); } const result = await executeDailyKitTransaction(kitToDeliver, retrievedBy, deliveredBy, date, time); setProducts((prev) => prev.map((p) => result.updatedProducts.find((u) => u.id === p.id) || p)); setMovements((prev) => [...result.movements, ...prev.filter((m) => !result.movements.some((r) => r.id === m.id))]); showToast(`⚡ Kit Diário baixado com sucesso! (${result.deliveredCount} itens)${saveAsDefault ? ' - modelo salvo!' : ''}`); }
    catch (err: any) { showToast(err.message || 'Não foi possível baixar o Kit Diário.', 'warning'); }
  };
  const handleSaveProduct = async (updatedProd: Product) => { try { await saveProductToFirestore(updatedProd); setProducts((prev) => prev.map((p) => p.id === updatedProd.id ? updatedProd : p)); showToast(`Produto ${updatedProd.name} atualizado!`, 'info'); } catch (e: any) { showToast(e.message, 'warning'); } };
  ...
  const handleForceSyncPhysicalInventory = async () => showToast('A sincronização física automática foi desativada para proteger o estoque real.', 'warning');
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans flex flex-col md:flex-row selection:bg-indigo-600 selection:text-white transition-colors duration-200">
      ...
      <main className="flex-1 min-w-0 p-4 sm:p-6 md:p-8 overflow-y-auto">
        <HeroAlertBanner products={products} onOpenWhatsApp={() => setIsWhatsAppModalOpen(true)} />
        {activeTab === 'dashboard' && <div className="space-y-6"><KpiCards products={products} movements={movements} /><CurrentStockOverview products={products} onOpenTimeline={handleOpenTimeline} onOpenEntry={handleOpenEntryModal} onOpenExit={handleOpenExitModal} userRole={currentUser.role} /><DashboardCharts products={products} movements={movements} /></div>}
        ...
        {activeTab === 'entries' && <div className="space-y-6"><div className="flex items-center justify-between bg-white border border-slate-200 rounded-3xl p-6 shadow-sm"><div><h2 className="text-lg font-bold text-slate-900">Entradas no Estoque (Compras e Doações)</h2><p className="text-xs text-slate-500">Rastreio de todos os mantimentos recebidos na Cristolândia</p></div>{currentUser.role === 'admin' && <button onClick={() => handleOpenEntryModal(null)} className="px-5 py-3 bg-emerald-600 text-white font-bold text-xs sm:text-sm rounded-2xl">+ Nova Entrada</button>}</div><MovementsHistory movements={movements.filter((m) => m.type === 'entrada')} products={products} userRole={currentUser.role} onOpenProductTimeline={handleOpenTimelineById} onOpenEntryForDate={(d) => handleOpenEntryModal(null, d)} onOpenExitForDate={(d) => handleOpenExitModal(null, d)} onUpdateMovement={handleUpdateMovement} onDeleteMovement={handleDeleteMovement} /></div>}
        {activeTab === 'exits' && <div className="space-y-6"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm"><div><h2 className="text-lg font-bold text-slate-900">Saídas do Estoque por Setor</h2><p className="text-xs text-slate-500">Entrega de mantimentos para a Cozinha, Casas e Eventos</p></div><div className="flex items-center gap-2"><button onClick={() => setIsKitModalOpen(true)} className="px-5 py-3 bg-blue-600 text-white font-bold text-xs sm:text-sm rounded-2xl flex items-center gap-2"><Utensils className="w-4 h-4" /><span>{currentUser.role === 'admin' ? '+ Kit Cozinha Diário' : 'Visualizar Kit Cozinha'}</span></button>{currentUser.role === 'admin' && <button onClick={() => handleOpenExitModal(null)} className="px-5 py-3 bg-slate-900 text-white font-bold text-xs sm:text-sm rounded-2xl">Nova Saída</button>}</div></div><MovementsHistory movements={movements.filter((m) => m.type === 'saida')} products={products} userRole={currentUser.role} onOpenProductTimeline={handleOpenTimelineById} onOpenEntryForDate={(d) => handleOpenEntryModal(null, d)} onOpenExitForDate={(d) => handleOpenExitModal(null, d)} onUpdateMovement={handleUpdateMovement} onDeleteMovement={handleDeleteMovement} /></div>}
        {activeTab === 'meals' && <MealManager meals={meals} missionaries={missionaries} userRole={currentUser.role} currentUserDisplayName={currentUser.displayName} onSaveMealRecord={handleSaveMealRecord} onDeleteMealRecord={handleDeleteMealRecord} />}
