...
// LocalStorage is cache only. It must never seed or overwrite authoritative Firestore stock.
...
// Legacy local helpers remain for historical compatibility only. App stock operations no longer call them.
const HOUSES_TO_COZINHA_NAMES = new Set<string>(['Casa Marcos & Fabíola (Cesta Básica)', 'Casa da Coordenação (Huberto & Débora)', 'Casa Lana & Joabe (Cesta Básica)', 'Casa Tainã (Cesta Básica)']);
function sanitizeStoredMovement(m: StockMovement): StockMovement { return m.sector && HOUSES_TO_COZINHA_NAMES.has(m.sector) ? { ...m, sector: 'Cozinha', kitchenShift: m.kitchenShift || 'Almoço' } : m; }
...
export function addOrUpdateMealRecord(allMeals: DailyMealRecord[], record: Omit<DailyMealRecord, 'id' | 'totalMeals' | 'createdAt'> & { id?: string; createdAt?: string }): { updatedMeals: DailyMealRecord[]; savedRecord: DailyMealRecord } {
  const now = new Date().toISOString(); const savedRecord: DailyMealRecord = { ...record, id: record.id || `meal-${Date.now()}`, totalMeals: Number(record.breakfast || 0) + Number(record.lunch || 0) + Number(record.afternoonSnack || 0) + Number(record.dinner || 0), createdAt: record.createdAt || now, updatedAt: now } as DailyMealRecord;
  return { savedRecord, updatedMeals: [savedRecord, ...allMeals.filter((m) => m.id !== savedRecord.id)] };
}
